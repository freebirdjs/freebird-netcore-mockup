...
Controller.prototype._DeviceLeave = function (permAddr) {
    ...
    if (dev)
        this.emit(EVTS.DEV_LEAVING, dev);
...
Controller.prototype._gadAttrRandomChanges = function (eventType, isAppend) {
    ...
    if (!pickedOid && eventType)
        return this._gadAttrRandomChanges(eventType, isAppend);
    ...
    if (rdef.access !== 'E') {
        ...
        this._gadAttrRandomChanges(eventType, isAppend);
    }
...
Controller.prototype.startEmitGadChanges = function () {
    var self = this;
    var randomDuration = chance.integer({ min: 500, max: 3000 });
...
Controller.prototype.devIdentify = function (permAddr, done) {
    if (done)
